...
export default function CardDB({ deploy, index }: props) {
  ...
  const handleClick = () => {
    router.push(`/dashboard/deploy/${deploy._id}?extras=${index}/${update}`);
  };
    ...
    const getCommit = async () => {
      try {
        ...
        await axiosAuth
          ...
          .then((data: { commit: { sha: any } }) => data.commit.sha)
          .then((commit: string | undefined) => {
            commit != deploy?.commits[auxCommit! - 1]?._id
              ? setUpdate(true)
              : setUpdate(false);
          });
        getColor();
  ...
  return (
    <div
      className="w-full max-w-[26rem] h-[17rem] bg-white mt-6 rounded-2xl mx-0 sm:mx-4 md:mx-2 lg:mx-3 shadow-xl z-10 overflow-hidden"
      onClick={handleClick}
    >
      <div className="px-6 py-4">
        <div className="relative flex space-x-4 items-center mb-6 justify-between">
          ...
          <div
            className={`absolute top-0 right-0 w-2.5 h-2.5 ${color} rounded-full`}
          ></div>
        </div>
        <div>
          <div className="flex justify-between pb-4 items-center">
            <div className="flex flex-wrap">
              <h2 className="text-sm font-bold text-gray-900 mr-2">
                Last commit
              </h2>
      ...
      <div className="flex flex-col items-center pt-1 px-4 py-8">
        ...
        <button
          className="flex bg-gray-100 rounded-2xl px-3 py-1 text-xs items-center"
          onClick={(event) => {
...
export function CardDBSkeleton() {
  return (
    <div className="w-full mt-6 h-[17rem] rounded-2xl px-2 sm:px-4 md:px-2 lg:px-3 shadow-lg z-10 bg-gray-200 animate-pulse"></div>
  );
}
