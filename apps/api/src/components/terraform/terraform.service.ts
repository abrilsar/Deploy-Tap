...
async function executeTerraform(reply: FastifyReply, request: FastifyRequest): Promise<void> {
  try {
    if (process.env.NODE_ENV as string === "production") {
      const terraform_init = childProcess.spawn('terraform', ['init'], { cwd: "terraform/create" });
      ...
      terraform_init.stdout.on('data', (data: Buffer) => {
        console.log(`Terraform init output: ${data.toString()}`);
      });

      terraform_init.stderr.on('data', (data: Buffer) => {
        console.error(`Terraform init error: ${data.toString()}`);
      });
    } else {
